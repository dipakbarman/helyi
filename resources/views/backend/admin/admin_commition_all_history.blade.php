  ...
  @include('backend.admin.tpage_menu')
    <div class="col-md-12">
        <div class="card p-2">
            ...
            <form action="{{ url('admin_commition_all_history_filter') }}" method="get">
              @csrf
          <div class="row mb-2">
              <div class="col-md-2">
              ...
              <div class="col-md-2">
                  <label for=""> </label>
                  <a href="{{ url('admin_commition_all_history') }}" type="submit" class="btn btn-success btn-block">Reset</a>
              </div>
          </div>
      </form>
            <div class="row">
                        ...
                        <table class="table">
                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td>{{ datedbu($list->getdate) }}</td>
                              <td>@if(!empty($list->time)) {{date('h:i:s a',$list->time)}} @endif</td>
                                <td>{{ $list->user_name }}</td>
                                <td>{{ $list->user_phone }}</td>
                                <td>{{ card_type_by_addmoney_id($list->add_money_id) }}</td>
                                <td>{{ number_format($list->distribute_amount,2) }}</td>
                                <td>{{ number_format($list->distributor,2) }}</td>
                                <td>{{ number_format($list->master_distributor,2) }}</td>
                                <td>{{ number_format($list->admin_total,2) }}</td>
                                <td>{{ $list->admin }}</td>
                                <td>{{ number_format(admin_count_prewallet($list->id),2) }}</td>
                            </tr>
                            @endforeach
                          </tbody>
                        </table>
